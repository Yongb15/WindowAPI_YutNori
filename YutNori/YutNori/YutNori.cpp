...
void DrawBoard(HDC hdc)
{
    ...
    // 오른쪽 패널 그리기
    int panelLeft = boardLeft + boardSize + 50;
    ...
    */
    ...
    case YutResult::DO: circlesRemoved = 1; break;
    case YutResult::GAE: circlesRemoved = 2; break;
    case YutResult::GEOL: circlesRemoved = 3; break;
    case YutResult::YUT: circlesRemoved = 4; break;
    case YutResult::MO: circlesRemoved = 0; break;
    }
    ...
    // 추가: 한 번 더 던질 수 있다는 메시지 표시
    if (gameState.canThrowAgain) {
        TextOut(hdc, sticksLeft, sticksTop + stickHeight + 50, L"한번 더!", 4);
    }
...
void HandleClick(HWND hWnd, int x, int y, GameState& state)
{
    ...
    if (x >= throwButton.left && x <= throwButton.right &&
        ...
        // 윷 던지기 결과를 메시지 박스로 표시
        MessageBox(hWnd, (L"윷 결과: " + resultStr).c_str(), L"윷 던지기 결과", MB_OK);
        ...
        for (auto& piece : currentPlayerPieces) {
            if (piece.isOnBoard && !piece.isFinished) {
                ...
                if (dx * dx + dy * dy <= NORMAL_RADIUS * NORMAL_RADIUS) {
                    // 말을 클릭했을 때의 처리
                    // 이 부분은 이제 사용되지 않습니다. 모든 이동은 자동으로 처리됩니다.
                    break;
                }
            }
        }
